...
if __name__ == "__main__" :
    #1~10 ave
    f1 = open("./data_sd/exp"+str(exp)+"_table_sd.txt","w")
    #1~10
    f2 = open("./data_sd/exp"+str(exp)+"_all_sd.txt","w")
    ...
    #data_test_plt = np.std(data_test_plt)
    print (data_test_plt[:,0])
    print (data_test_plt[:,1])
    ...
    for i in range(SEQ):
        f2.write("%d " %(i+1))
        f2.write("%g" %(np.std(data_test_plt[:,i])))
        f2.write("\n")
    ...
    for i in range(SEQ):
        f1.write("%g " %(np.std(data_test_plt[:,i])))
